...
**用户视角**的速查手册——不讲 YAML，只讲"在哪敲、怎么敲、会看到什么、失败怎么办"：

- 一张**命令速查表**：9 个命令 × 触发位置 × 权限要求 × 响应时间，一眼看全
- 每条命令的**完整交互示例**（你敲了啥、机器人回了啥、中间 emoji 反应怎么变）
- 三个最容易踩的坑：**命令位置搞混**（issue vs PR）、**权限不够被静默忽略**、**幻觉三大重灾区**（版本号 / 链接 / API 签名）
- 如何禁止 AI 碰某些文件：`BLOCKED` regex 的加法
- 日常收藏为浏览器书签型手册
